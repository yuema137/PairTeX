...
跟 coding agent 一起改论文，现在多半是这么个来回：编译出 PDF，自己读，再把意见
一条条复制回聊天框，稿子和意见压根不在一个地方。PairTeX 把这一圈接上了，它把你
现有的 LaTeX 仓库渲染成一个 HTML 页面，批注和修改都在页面上直接做，每个动作记成
一个结构化的反馈文件，你的 agent 直接读这些文件。

PairTeX 就是这么一层评审界面：小、确定、随时可插拔，加在已经有的 LaTeX 项目上。
